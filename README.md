...
## Headline results (Tesla T4, real hardware)

The wins and the losses, honestly:

* **Continuous batching + paged KV (Phase 5): 12.8× throughput** — 86 tok/s
  sequential naive → 1099 tok/s at batch 32. The architectural optimizations
  are where the real serving wins live.
* **Speculative decoding (Phase 6): 1.7× latency win** — 113 → 192 tok/s at
  γ=4 with 45% draft acceptance, output token-identical to the target.
* **Flash attention (Phase 1): the memory story is dramatic, the latency
  story is not.** At S=4096: naive attention needs 4.2 GB for its score
  matrix, the fused kernel 73 MB (58×). But on latency the first-pass Triton
  kernel loses to cuBLAS-backed naive at long S and to SDPA everywhere —
  T4 (sm_75) is a weak Triton target (no async copy, small SMEM, and this
  kernel has no autotuning). On Ampere+ the same code would fare far better.
* **Fused dequant-matmul (Phase 4): correct but slower than cuBLAS fp16**
...
* **All 9 GPU kernel-correctness smoke checks pass** on compiled Triton —
  every benchmark above ran only after correctness was verified on-device.
